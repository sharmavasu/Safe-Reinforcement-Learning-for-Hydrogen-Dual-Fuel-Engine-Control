...
def retrieve_episode(
    ...
    # print(IMEP[0][0])
    # print(IMEP[0][1])
    controller.run_cmd("experience_buffer.get('NOX');")
    NOX = controller.read_workspace_var("ans")


    controller.run_cmd("experience_buffer.get('soot');")
    SOOT = controller.read_workspace_var("ans")
  

    controller.run_cmd("experience_buffer.get('MPRR');")
    MPRR = controller.read_workspace_var("ans")
 
    controller.run_cmd("experience_buffer.get('DOI_main');")
    DOIMain = controller.read_workspace_var("ans")
 

    controller.run_cmd("experience_buffer.get('SOI_pre');")
    P2M = controller.read_workspace_var("ans")
    
    controller.run_cmd("experience_buffer.get('SOI_main');")
    SOIMain = controller.read_workspace_var("ans")

    controller.run_cmd("experience_buffer.get('DOI_H2');")
    DOIH2 = controller.read_workspace_var("ans")
    ...
    for i in range(1, len(norm_obs)):
        ...
        if algorithm == "ddpg":
            aux_data = {
                ...
                "NOX_actual": NOX[i][1],
                "SOOT_actual": SOOT[i][1],
                "MPRR_actual": MPRR[i][1],
                "DOIMain": DOIMain[i][0],
                "P2M": P2M[i][0],
                "SOIMain": SOIMain[i][0],
                "DOIH2": DOIH2[i][0],
            ...
            aux_data = {
                ...
                "norm_action3_dist[0]": norm_action_dist[2][0][i],
                "norm_action4_dist[0]": norm_action_dist[3][0][i],
                "norm_action1_dist[0]": norm_action_dist[4][0][i],
                "norm_action2_dist[0]": norm_action_dist[5][0][i],
                "norm_action3_dist[0]": norm_action_dist[6][0][i],
                "norm_action4_dist[0]": norm_action_dist[7][0][i],
                "IMEP_ref" : IMEP[i][0],
                "IMEP_actual": IMEP[i][1],
                "NOX_actual": NOX[i][1],
                "SOOT_actual": SOOT[i][1],
...
def prepare_env(
    ...
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/norm_observation_size",
        16,
    )
    if algorithm == "ddpg":
        controller.write_simulink_var(
            "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/norm_action_dist_size",
            4,
        )
    elif algorithm == "ppo":
        controller.write_simulink_var(
    ...
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/static_nn_memory",
        f"uint8({list(nn_bytes)})",
    )
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/tensor_arena_size",
        100000,
    )
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/lower_observation_bounds",
        [-20, -20, -20, -20, -20, -20, -20, -20, -20, -20, -20, -20,-20, -20, -20, -20],
    )
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/upper_observation_bounds",
        [+20, +20, +20, +20, +20, +20, +20, +20, +20, +20, +20, +20, +20, +20, +20, +20],
    )
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/lower_action_bounds",
        [-2.3201,   -2.1257,   -1.8309,   -1.6562],
    )
    controller.write_simulink_var(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/upper_action_bounds",
        [1.4113,    0.9867,    0.9542,    2.1874],
    )
    if is_training_episode:
        controller.write_simulink_var(
            "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/b_training_active",
            1,
        )
    else:
        controller.write_simulink_var(
    ...
    if algorithm == "ddpg":
        ...
        controller.write_simulink_var(
            "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/ddpg_standard_deviation",
            calc_stdev(initial_stdev, stdev_decay_factor, cycle_no),
        )

    # Set the seeds of the RNGs
    controller.write_simulink_block_param(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/Sampling/Continuous Action/DDPG_Sampler/Random\nNumber",
        "Seed",
        int(time.time()),
    )
    controller.write_simulink_block_param(
        "H2DFRL_LExCI_env_PPO/RL_Block/RL_Agent/Sampling/Continuous Action/PPO_Sampler/Random\nNumber",
        "Seed",
        int(time.time()),
    )
    ...
    if is_training_episode:
        ...
        controller.write_simulink_var(
            r"H2DFRL_LExCI_env_PPO/RAND0",
            np.random.uniform(4e5,10e5),
        )

        controller.write_simulink_var(
            r"H2DFRL_LExCI_env_PPO/RAND1",
            np.random.uniform(0.5,1.3),
        )

        controller.write_simulink_var(
            r"H2DFRL_LExCI_env_PPO/RAND2",
            np.random.uniform(0.75,1.4),
        )

        controller.write_simulink_var(
            r"H2DFRL_LExCI_env_PPO/RAND3",
...
def generate_training_data(
    ...
    while num_collected_exps < num_experiences:
        ...
        controller.run_cmd("experience_buffer = sim('H2DFRL_LExCI_env_PPO');")
...
if __name__ == "__main__":
    ...
    controller.start_matlab_simulink(
        "/home/vasu3/Documents/work/h2df_rl_lexci/rl_uofa_pretraining/H2DFRL_LExCI_PPO/",
        "H2DFRL_LExCI_env_PPO",
