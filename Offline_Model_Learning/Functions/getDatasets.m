function [utrain, ytrain, uval, yval, utest, ytest] = getDatasets(data_conc, idx_start, idx_end, ratio_train, ratio_val, plot_init, kill_violated_data, kill_points_zero_h2_doi, positive_IMEP, RNG)
...
if kill_violated_data
    ...
    % if positive_IMEP
    %         valid_indices = find(IMEP_cycle > 0);
    % 
    %         % Keep only the indices with positive IMEP
    %         H2_doi_cycle = H2_doi_cycle(valid_indices);
    %         DOI_main_cycle = DOI_main_cycle(valid_indices);
    %         SOI_main_cycle = SOI_main_cycle(valid_indices);
    %         P2M_cycle = P2M_cycle(valid_indices);
    %         NOx_cycle = NOx_cycle(valid_indices);
    %         Soot_cycle = Soot_cycle(valid_indices);
    %         IMEP_cycle = IMEP_cycle(valid_indices);
    %         MPRR_cycle = MPRR_cycle(valid_indices);
    %     end
else
    ...
    if kill_points_zero_h2_doi
        ...
        % New flag to remove negative IMEP values
