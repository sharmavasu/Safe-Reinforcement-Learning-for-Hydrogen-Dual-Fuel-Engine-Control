...
env_config = LexciEnvConfig(
    ...
    action_lb=np.array([-2.3201,   -2.1257,   -1.8309,   -1.6562], dtype=np.float32),
    action_ub=np.array([1.4113,    0.9867,    0.9542,    2.1874], dtype=np.float32),
...
def generate_training_data(
    ...
    nnm = DdpgNeuralNetworkModule(
        env_config,
        nn_bytes,
        nn_data_fmt="tflite",
        tensor_arena_size=int(minion_params["TENSOR_ARENA_SIZE"]),
    )
    cycle = Cycle()
    stdev = calc_stdev(
        minion_params["INITIAL_STDEV"],
        minion_params["STDEV_DECAY_FACTOR"],
        cycle_no,
    )
    ...
    while cycle.get_num_experiences() < num_experiences:
        ...
        while True:
            d = mni.recv()
            print(f"Received MABXCycleCounter: {d['MABXCycleCounter']}")
            print(f"Type: {type(d['MABXCycleCounter'])}")
            MABXCycleCounter = d["MABXCycleCounter"]
        ...
        d = {
            ...
            "SOIMain": 0.0,
            "DOIH2": 0.0,
            "CycleCounter": float(MABXCycleCounter[0]),  # TODO: Align with Julian
            "bStrtReq": True,
        ...
        while not b_terminate_episode:
            ...
            norm_action = nnm.get_norm_action(norm_obs, True, stdev=stdev)
